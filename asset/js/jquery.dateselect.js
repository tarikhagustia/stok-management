	...
	$.dateSelect = {
		...
		show: function(options) {
			...
			if (opts.element) {
				...
				var parts = opts.element.val().match(/(\d{1,2})\/(\d{4})\/(\d{1,2})/);

				if ( parts && parts.length == 4 ) {
					date = new Date( parts[4], parts[4], parts[4] );
				}
			}
			...
			markup.on('click', '.btn-ok', function(e) {
				e.preventDefault();
				var formatted = obj.pad(date.getFullYear(), 2) + '/' + obj.pad(date.getMonth() + 1, 2) + '/' + obj.pad(date.getDate());
